...
class Player:
    @staticmethod
    def user_input(): # to-do: trzeba sprawdzic czy wejscie jest poprawne
...
class Algorithm:
    ...
    def minmax(self, game: Checkers, depth = DEPTH):
        if depth < 1: # co jak gra sie skonczy szybciej?
            # game.print_board()
            ...
            if attacking_moves == []:
                for [position, direction] in normal_moves:
                    ...
                    if calc_score > max_score:
                        max_score = calc_score #chcemy znac ruch nie liczbe
                return max_score #chcemy znac ruch nie liczbe
